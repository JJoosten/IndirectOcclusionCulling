...
inline void DebugLog(char const* const Format, ...)
{
#ifdef ENABLE_DEBUG_LOG
    static int const BufferSize = 65536;
    static char* WideBuffer = nullptr;

    if (!WideBuffer)
    {
        WideBuffer = new char[BufferSize];
    }

    va_list Args;
    va_start(Args, Format);
    _vsnprintf_s(WideBuffer, BufferSize, BufferSize - 1, Format, Args);
    va_end(Args);

    OutputDebugStringA(WideBuffer);
#endif
}

inline void ReleaseLog(char const* const Format, ...)
{
    ...
    _vsnprintf_s(WideBuffer, BufferSize, BufferSize, Format, Args);
    va_end(Args);

...
/*
 ...
 - limited layout gpu_features, intricate layouts are typically crafted in code
 ...
 - CTRL+Click on a slider or drag gpu_box to input value as text
 - text editor:
 ...
 - call and read ImGui::ShowTestWindow() for demo code demonstrating most gpu_features.
 ...
 - 2015/12/27 (1.48) - fixed ImDrawList::AddRect() which used to render a gpu_rectangle 1 px too large on each axis.
...
!- window: begin with *p_open == false should return false.
 ...
 - draw-list: maintaining bounding gpu_box per command would allow to merge draw command when clipping isn't relied on (typical non-scrolling window or non-overflowing column would merge with previous command).
...
!- color: the color helpers/typing is a mess and needs sorting out.
 ...
 - style: color-gpu_box not always square?
 ...
 - style: WindowPadding needs to be EVEN needs the 0.5 multiplier probably have a subtle effect on clip gpu_rectangle
...
!- keyboard: tooltip & combo boxes are messing up / not honoring keyboard tabbing
 ...
 - optimization/render: merge command-lists with same clip-rect into one even if they aren't sequential? (as long as in-between clip gpu_rectangle don't overlap)?
...
ImGuiStyle::ImGuiStyle()
{
    ...
    FramePadding            = ImVec2(4,3);      // Padding within a framed gpu_rectangle (used by most widgets)
    FrameRounding           = 0.0f;             // Radius of frame corners rounding. Set to 0.0f to have rectangular frames (used by most widgets).
    ItemSpacing             = ImVec2(8,4);      // Horizontal and vertical spacing between widgets/lines
    ItemInnerSpacing        = ImVec2(4,4);      // Horizontal and vertical spacing between within elements of a composed widget (e.g. a slider and its label)
    TouchExtraPadding       = ImVec2(0,0);      // Expand reactive bounding gpu_box for touch-based system where touch position is not accurate enough. Unfortunately we don't sort widgets so priority on overlap will always be given to the first widget. So don't grow this too much!
    IndentSpacing           = 21.0f;            // Horizontal spacing when e.g. entering a tree node. Generally == (FontSize + FramePadding.x*2).
    ColumnsMinSpacing       = 6.0f;             // Minimum horizontal spacing between two columns
    ScrollbarSize           = 16.0f;            // Width of the vertical scrollbar, Height of the horizontal scrollbar
    ScrollbarRounding       = 9.0f;             // Radius of grab corners rounding for scrollbar
    GrabMinSize             = 10.0f;            // Minimum width/height of a grab gpu_box for slider/scrollbar
...
// Declare item bounding gpu_box for clipping and interaction.
...
// Handle clipping on CPU immediately (vs typically let the GPU clip the triangles that are overlapping the clipping gpu_rectangle edges)
...
// Render a gpu_rectangle shaped with optional rounding and borders
...
// FIXME: Note that we have a lag here because WindowRectClipped is updated in Begin() so windows moved by user via SetWindowPos() and not SetNextWindowPos() will have that gpu_rectangle lagging by a frame at the time FindHoveredWindow() is called, aka before the next Begin(). Moving window thankfully isn't affected.
static ImGuiWindow* FindHoveredWindow(ImVec2 pos, bool excluding_childs)
{
    ImGuiContext& g = *GImGui;
...
// Test if mouse cursor is hovering given gpu_rectangle
// NB- Rectangle is clipped by our current clip setting
// NB- Expand the gpu_rectangle to be generous on imprecise inputs systems (g.Style.TouchExtraPadding)
...
bool ImGui::Begin(const char* name, bool* p_open, const ImVec2& size_on_first_use, float bg_alpha, ImGuiWindowFlags flags)
{
    ...
    if (first_begin_of_the_frame)
    {
        ...
        // Clear draw list, setup texture, outer clipping gpu_rectangle
        ...
        // At this point we don't have a clipping gpu_rectangle setup yet, so we can use the title bar area for hit detection and drawing
    ...
    // Inner clipping gpu_rectangle
    // We set this up after processing the resize grip so that our clip gpu_rectangle doesn't lag by a frame
    // Note that if our window is collapsed we will end up with a null clipping gpu_rectangle which is the correct behavior.
...
void ImGui::End()
{
    ...
    PopClipRect();   // inner window clip gpu_rectangle
...
static void Scrollbar(ImGuiWindow* window, bool horizontal)
{
    ...
    // The grabable gpu_box size generally represent the amount visible (vs the total scrollable amount)
...
bool ImGui::SliderFloat(const char* label, float* v, float v_min, float v_max, const char* display_format, float power)
{
    ...
    // NB- we don't call ItemSize() yet because we may turn into a text edit gpu_box below
    ...
    // Tabbing or CTRL-clicking on Slider turns it into an input gpu_box
...
bool ImGui::DragFloat(const char* label, float* v, float v_speed, float v_min, float v_max, const char* display_format, float power)
{
    ...
    // Tabbing or CTRL-clicking on Drag turns it into an input gpu_box
    bool start_text_input = false;
    const bool tab_focus_requested = FocusableItemRegister(window, g.ActiveId == id);
...
void ImGui::PlotEx(ImGuiPlotType plot_type, const char* label, float (*values_getter)(void* data, int idx), void* data, int values_count, int values_offset, const char* overlay_text, float scale_min, float scale_max, ImVec2 graph_size)
{
    ...
    float t0 = 0.0f;
    ImVec2 tp0 = ImVec2( t0, 1.0f - ImSaturate((v0 - scale_min) / (scale_max - scale_min)) );    // Point in the normalized space of our target gpu_rectangle
...
bool ImGui::InputTextEx(const char* label, char* buf, int buf_size, const ImVec2& size_arg, ImGuiInputTextFlags flags, ImGuiTextEditCallback callback, void* user_data)
{
    ...
    if (g.ActiveId == id)
    {
        ...
        else
        {
            ...
            // Note that as soon as the input gpu_box is active, the in-widget value gets priority over any underlying modification of the input buffer
...
// Combo gpu_box helper allowing to pass an array of strings.
...
// Combo gpu_box helper allowing to pass all items in a single string.
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags, const ImVec2& size_arg)
{
    ...
    // Selectables are tightly packed together, we extend the gpu_box to cover spacing between selectable.
...
// Lock horizontal starting position + capture group bounding gpu_box into one "item" (so you can use IsItemHovered() or layout primitives such as SameLine() on whole group, etc.)

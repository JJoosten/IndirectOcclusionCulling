...
namespace ImGui
{
    ...
    IMGUI_API void          ShowTestWindow(bool* p_open = NULL);        // test window demonstrating ImGui gpu_features
    ...
    IMGUI_API void          BeginGroup();                                                       // lock horizontal starting position + capture group bounding gpu_box into one "item" (so you can use IsItemHovered() or layout primitives such as SameLine() on whole group, etc.)
    ...
    // Widgets: Drags (tip: ctrl+click on a drag gpu_box to input with keyboard. manually input values aren't clamped, can go off-bounds)
    ...
    IMGUI_API bool          IsRectVisible(const ImVec2& size);                                  // test if gpu_rectangle of given size starting from cursor pos is visible (not clipped). to perform coarse clipping on user's side (as an optimization)
    IMGUI_API bool          IsPosHoveringAnyWindow(const ImVec2& pos);                          // is given position hovering any active imgui window
    IMGUI_API float         GetTime();
    IMGUI_API int           GetFrameCount();
    IMGUI_API const char*   GetStyleColName(ImGuiCol idx);
    IMGUI_API ImVec2        CalcItemRectClosestPoint(const ImVec2& pos, bool on_edge = false, float outward = +0.0f);   // utility to find the closest point the last item bounding gpu_rectangle edge. useful to visually link items
...
enum ImGuiTreeNodeFlags_
{
    ...
    ImGuiTreeNodeFlags_OpenOnArrow          = 1 << 7,   // Only open when clicking on the arrow part. If ImGuiTreeNodeFlags_OpenOnDoubleClick is also set, single-click arrow or double-click all gpu_box to open.
    ImGuiTreeNodeFlags_Leaf                 = 1 << 8,   // No collapsing, no arrow (use as a convenience for leaf nodes). 
    ImGuiTreeNodeFlags_Bullet               = 1 << 9,   // Display a bullet instead of arrow
    //ImGuITreeNodeFlags_SpanAllAvailWidth  = 1 << 10,  // FIXME: TODO: Extend hit gpu_box horizontally even if not framed
...
struct ImGuiStyle
{
    ...
    ImVec2      FramePadding;               // Padding within a framed gpu_rectangle (used by most widgets)
    float       FrameRounding;              // Radius of frame corners rounding. Set to 0.0f to have rectangular frame (used by most widgets).
    ImVec2      ItemSpacing;                // Horizontal and vertical spacing between widgets/lines
    ImVec2      ItemInnerSpacing;           // Horizontal and vertical spacing between within elements of a composed widget (e.g. a slider and its label)
    ImVec2      TouchExtraPadding;          // Expand reactive bounding gpu_box for touch-based system where touch position is not accurate enough. Unfortunately we don't sort widgets so priority on overlap will always be given to the first widget. So don't grow this too much!
    float       IndentSpacing;              // Horizontal indentation when e.g. entering a tree node. Generally == (FontSize + FramePadding.x*2).
    float       ColumnsMinSpacing;          // Minimum horizontal spacing between two columns
    float       ScrollbarSize;              // Width of the vertical scrollbar, Height of the horizontal scrollbar
    float       ScrollbarRounding;          // Radius of grab corners for scrollbar
    float       GrabMinSize;                // Minimum width/height of a grab gpu_box for slider/scrollbar
...
struct ImDrawCmd
{
    ...
    ImVec4          ClipRect;               // Clipping gpu_rectangle (x1, y1, x2, y2)
...
struct ImDrawList
{
    ...
    IMGUI_API void  PrimRect(const ImVec2& a, const ImVec2& b, ImU32 col);      // Axis aligned gpu_rectangle (composed of two triangles)

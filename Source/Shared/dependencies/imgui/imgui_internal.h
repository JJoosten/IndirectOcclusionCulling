...
// You may use this file to debug, understand or extend ImGui gpu_features but we don't provide any guarantee of forward compatibility!
...
// 2D axis aligned bounding-gpu_box
...
// Internal state of the currently focused/edited text input gpu_box
...
struct ImGuiContext
{
    ...
    ImVec2                  ScrollbarClickDeltaToGrabCenter;    // Distance between mouse and center of grab gpu_box, normalized in parent space. Use storage?
    char                    Tooltip[1024];
...
struct IMGUI_API ImGuiDrawContext
{
    ...
    ImGuiID                 LastItemID;
    ImRect                  LastItemRect;
    bool                    LastItemHoveredAndUsable;  // Item gpu_rectangle is hovered, and its window is currently interactable with (not blocked by a popup preventing access to the window)
    bool                    LastItemHoveredRect;       // Item gpu_rectangle is hovered, but its window may or not be currently interactable with (might be blocked by a popup preventing access to the window)
...
struct IMGUI_API ImGuiWindow
{
    ...
    ImRect                  ClipRect;                           // = DrawList->clip_rect_stack.back(). Scissoring / clipping gpu_rectangle. x1, y1, x2, y2.

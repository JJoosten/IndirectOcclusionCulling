...
MICROPROFILE_API MicroProfileThreadLogGpu* MicroProfileGetGlobalGpuThreadLog();
...
void MicroProfileGpuShutdown()
{
	...
	S.GPU.pHeap = nullptr;
	S.GPU.pBuffer = nullptr;
	S.GPU.pFence = nullptr;
	for (MicroProfileD3D12Frame& Frame : S.GPU.Frames)
	{
		if(Frame.pCommandAllocator)
			Frame.pCommandAllocator->Release();
		if(Frame.pCommandList)
			Frame.pCommandList->Release();
		Frame.pCommandAllocator = nullptr;
		Frame.pCommandList = nullptr;
	}
}
